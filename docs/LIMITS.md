...
The default matrix in DESIGN §5.5 ships as **v0 proposals**. Use `prismshine calibrate` (synthetic or labeled) and keep the overlay’s `calibration_version` in the verdict cache key. Public accuracy claims require the benchmark gates in `POSITIONING.md`.
